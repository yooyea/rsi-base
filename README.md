...
本仓库也使用自己的准则。具体风险、工程载体、验证入口及尚未覆盖的部分见[自举记录](docs/self-bootstrap.md)。
...
离线检查覆盖本地文档路径、Skill 基本元数据、准则章节正文及从入口到所有准则的可达性。它不证明准则语义正确、外部链接在线、工程保障已实施或业务已通过验收。

## 当前边界

这是首版指导，尚未在业务项目验证效果。本仓库的[期望门禁](.github/rulesets/main.json)进入版本管理；实际生效状态使用只读审计与隔离合入实验核实，不能仅凭此文件宣布启用。
